...
import { getChannelDocument } from "../graphql/queries";
import { updateChannelDocument } from "../graphql/mutations";
export default function ChannelDocumentUpdateForm(props) {
  const {
    id: idProp,
    channelDocument: channelDocumentModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = channelDocumentRecord
      ? { ...initialValues, ...channelDocumentRecord }
      : initialValues;
    setChannelId(cleanValues.channelId);
    setTitle(cleanValues.title);
    setS3KeyRawText(cleanValues.s3KeyRawText);
    setS3KeyOriginal(cleanValues.s3KeyOriginal);
    setDescription(cleanValues.description);
    setIncludeInLibrary(cleanValues.includeInLibrary);
    ...
    setFileSuffix(cleanValues.fileSuffix);
    setFileSize(cleanValues.fileSize);
    setUploadedBy(cleanValues.uploadedBy);
    setErrors({});
  };
  const [channelDocumentRecord, setChannelDocumentRecord] = React.useState(
    channelDocumentModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getChannelDocument.replaceAll("__typename", ""),
              variables: { ...idProp },
            })
          )?.data?.getChannelDocument
        : channelDocumentModelProp;
      setChannelDocumentRecord(record);
    };
    queryData();
  }, [idProp, channelDocumentModelProp]);
  React.useEffect(resetStateValues, [channelDocumentRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          channelId,
          title,
          s3KeyRawText: s3KeyRawText ?? null,
          s3KeyOriginal: s3KeyOriginal ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateChannelDocument.replaceAll("__typename", ""),
            variables: {
              input: {
                channelId: channelDocumentRecord.channelId,
                id: channelDocumentRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ChannelDocumentUpdateForm")}
      {...rest}
    >
      <TextField
        label="Channel id"
      ...
      >
        ...
        <option
          children="Text"
          value="TEXT"
          {...getOverrideProps(overrides, "documentTypeoption4")}
        ></option>
      </SelectField>
      <TextField
        label="Source"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || channelDocumentModelProp)}
